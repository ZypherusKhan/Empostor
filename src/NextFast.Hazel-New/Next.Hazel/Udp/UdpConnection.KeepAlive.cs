...
partial class UdpConnection
{
    ...
    private async void HandleKeepAlive(object state)
    {
        ...
        if (pingsSinceAck >= MissingPingsUntilDisconnect)
        {
            ...
            await DisconnectInternal(HazelInternalErrors.PingsWithoutResponse,
    ...
    private async ValueTask SendPing()
    {
        ...
        PingPacket pkt;
        if (!activePingPackets.TryGetValue(id, out pkt))
        {
            pkt = PingPacket.GetObject();
            if (!activePingPackets.TryAdd(id, pkt)) throw new Exception("This shouldn't be possible");
        }
    ...
    private void DisposeKeepAliveTimer()
    {
        ...
        foreach (var kvp in activePingPackets)
            if (activePingPackets.TryRemove(kvp.Key, out var pkt))
                pkt.Recycle();
    }
    ...
    {
        private static readonly ObjectPoolCustom<PingPacket> PacketPool = new(() => new PingPacket());
