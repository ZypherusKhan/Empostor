...
public class UdpClientConnection : UdpConnection
{
    ...
    private readonly Timer reliablePacketTimer;
    ...
    /// </summary>
    public UdpClientConnection(IPEndPoint remoteEndPoint, ObjectPool<MessageReader> readerPool,
        IPMode ipMode = IPMode.IPv4) : base(null, readerPool)
    {
    ...
    private async ValueTask WriteBytesToConnectionReal(byte[] bytes, int length)
    {
        ...
        {
            await DisconnectInternal(HazelInternalErrors.SocketExceptionSend,
                "Could not send data as a SocketException occurred: " + ex.Message);
        }
    }
    ...
    private async Task ListenAsync()
    {
        ...
        while (State != ConnectionState.NotConnected)
        {
            ...
            {
                await DisconnectInternal(HazelInternalErrors.SocketExceptionReceive,
                    "Socket exception while reading data: " + e.Message);
                return;
            }
            catch (Exception)
            {
    ...
    {
        if (state == ConnectionState.Connected) _connectWaitLock.Release();
    }
    ...
    /// </summary>
    protected override async ValueTask<bool> SendDisconnect(MessageWriter data)
    {
        lock (this)
        {
