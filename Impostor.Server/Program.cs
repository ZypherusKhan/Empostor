        ...
        private static IHostBuilder CreateHostBuilder(string[] args)
        {
            ...
            var hostBuilder = Host.CreateDefaultBuilder(args)
                ...
                .ConfigureServices((host, services) =>
                {
                    ...
                    services.Configure<AuthConfig>(host.Configuration.GetSection(AuthConfig.Section));
                    services.AddSingleton<DtlsCertificateService>();
                    services.AddSingleton<IpFriendCodeCache>();
                    services.AddSingleton<FriendCodeNonceStore>(sp =>
                    {
                        var authCfg = host.Configuration
                            .GetSection(AuthConfig.Section)
                            .Get<AuthConfig>() ?? new AuthConfig();
                        var logger = sp.GetRequiredService<Microsoft.Extensions.Logging.ILogger<FriendCodeNonceStore>>();
                        return new FriendCodeNonceStore(logger, authCfg.NonceTtlSeconds);
                    });
                ...
                .UseSerilog((context, loggerConfiguration) =>
                {
                    ...
                    static Assembly? LoadSerilogAssembly(AssemblyLoadContext loadContext, AssemblyName name)
                    {
                        ...
                        {
                            try { return loadContext.LoadFromAssemblyPath(Path.Combine(path, name.Name + ".dll")); }
            ...
            if (httpConfig.Enabled)
            {
                hostBuilder.ConfigureWebHostDefaults(builder =>
                {
                    ...
                    builder.Configure(app =>
                    {
                        ...
                        foreach (var p in pluginLoaderService.Plugins)
